...
const STORAGE_PREFIX = 'dermatologist_v4_'
...
export function usePlatformState() {
  ...
  useEffect(() => {
    ...
    if (DEV_AUTO_LOGIN) {
      ...
      const savedAuthData = localStorage.getItem('dermatologistMemberAuth')
      ...
      if (savedAuthData) {
        try {
          ...
          if (authData.expiry && Date.now() < authData.expiry) {
            ...
            // Expired - clear auth
            localStorage.removeItem('dermatologistMemberAuth')
          }
        } catch {
      ...
      if (!isValidAuth) {
        const sessionAuth = sessionStorage.getItem('dermatologistMemberAuth')
  ...
  const handleLogin = useCallback(async (email: string, password: string, rememberMe: boolean = true): Promise<boolean> => {
    ...
    try {
      const response = await fetch('/api/dermatologist-auth', {
        method: 'POST',
      ...
      if (data.success) {
        ...
        if (rememberMe) {
          ...
          localStorage.setItem('dermatologistMemberAuth', JSON.stringify({
            authenticated: true,
            expiry: thirtyDaysFromNow
          }))
        } else {
          // Session only - use sessionStorage (cleared when browser closes)
          sessionStorage.setItem('dermatologistMemberAuth', 'true')
          localStorage.removeItem('dermatologistMemberAuth')
        }
        ...
        if (data.isVip && data.vipGuest) {
          ...
          localStorage.setItem('dermatologistMemberEmail', data.user.email)
        } else {
          setVipGuest(null)
        }
  ...
  const handleLogout = useCallback(() => {
    ...
    setVipGuest(null)
    localStorage.removeItem('dermatologistMemberAuth')
    sessionStorage.removeItem('dermatologistMemberAuth')
