...
export default function PsychologistUpsellPage() {
  ...
  useEffect(() => {
    ...
    if (userId) {
      sessionStorage.setItem('psychologist_purchase_user_id', userId)
    }
  }, [])

  // Get the 5 upsell products
  const upsellProducts = getPsychologistUpsellBonuses()
  ...
  const handleDecline = () => {
    window.location.href = "/psychologists/downsell"
  }

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-purple-950/20 to-slate-950 flex items-center justify-center p-2 md:p-3">
