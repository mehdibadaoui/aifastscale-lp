...
export default function PlasticSurgeonUpsellPage() {
  ...
  useEffect(() => {
    ...
    if (userId) {
      sessionStorage.setItem('plastic_surgeon_purchase_user_id', userId)
    }
  }, [])

  // Get the 5 upsell products
  const upsellProducts = getPlasticSurgeonUpsellBonuses()
  const totalOriginalValue = upsellProducts.reduce((sum, b) => sum + b.value, 0)
  const upsellPrice = 47.82
  const pricePerItem = 9.56
  ...
  const handleDecline = () => {
    window.location.href = "/plastic-surgeons/downsell"
  }

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-[#1a0a2e] to-slate-950 flex items-center justify-center p-2 md:p-3">
      ...
      <div className="w-full max-w-md bg-gradient-to-br from-slate-900 to-slate-800 rounded-xl md:rounded-2xl border border-purple-500/30 shadow-2xl shadow-purple-500/10 overflow-hidden">
        ...
        {/* Header - Timer */}
        <div className="bg-gradient-to-r from-purple-500 to-amber-500 px-3 md:px-4 py-2 md:py-2.5">
        ...
        <div className="p-3 md:p-4 max-h-[85vh] overflow-y-auto">
          ...
          <div className="text-center mb-3 md:mb-4">
            ...
            <p className="text-white/60 text-xs md:text-sm">
              Add 5 more premium tools for just <span className="text-purple-400 font-black">${pricePerItem} each</span>
            </p>
          </div>

          {/* Products Card */}
          <div className="bg-white/5 rounded-lg md:rounded-xl border border-purple-500/30 mb-3 md:mb-4">
            <div className="px-2.5 md:px-3 py-1.5 md:py-2 border-b border-white/10">
              <div className="flex items-center justify-between">
                ...
                <span className="text-purple-400 font-black text-xs md:text-sm">${pricePerItem}/each</span>
              </div>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1 md:gap-1.5 flex-shrink-0">
                      ...
                      <span className="text-purple-400 font-black text-[10px] md:text-xs bg-purple-500/20 px-1 md:px-1.5 py-0.5 rounded">FREE</span>
                    </div>
                  </div>
                )
              })}
          ...
          <div className="bg-white/5 rounded-lg md:rounded-xl p-3 md:p-4 mb-3 md:mb-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1.5 md:gap-2">
                <span className="text-2xl md:text-3xl font-black text-purple-400">${upsellPrice}</span>
                <span className="bg-purple-500/20 text-purple-400 text-[10px] md:text-xs px-1.5 md:px-2 py-0.5 md:py-1 rounded font-black">
                  ${pricePerItem}/tool
                </span>
              </div>
            </div>
          ...
          <div className="space-y-2 md:space-y-3">
            <a
              ...
              className="w-full bg-gradient-to-r from-purple-500 to-amber-500 text-white py-4 md:py-5 rounded-xl font-black text-base md:text-lg flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-[0.98] transition-transform shadow-lg shadow-purple-500/30"
            >
