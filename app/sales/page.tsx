...
export default function LuxuryLanding() {
  ...
  useEffect(() => {
    const targetValue = selectedBonuses.reduce((sum, id) => {
    ...
    const animate = () => {
      ...
      setAnimatedValue(Math.round(startValue + (targetValue - startValue) * easeProgress))
  ...
  const toggleBonus = (id: string) => {
    ...
    if (selectedBonuses.includes(id)) {
      ...
      // Selecting
      const newSelection = [...selectedBonuses, id]
      setSelectedBonuses(newSelection)
      setJustSelected(id)

      // Clear the "just selected" animation after delay
      setTimeout(() => setJustSelected(null), 600)

      // Trigger haptic feedback
      if (newSelection.length === 5) {
        // Celebration for completing selection!
        triggerHaptic('success')
        setShowConfetti(true)
        setTimeout(() => setShowConfetti(false), 3000)
        // No auto-scroll - sticky bar shows checkout button instantly
      } else {
  ...
  return (
    <div className="min-h-screen bg-luxury-black font-dm-sans text-white luxury-page-enter">
        ...
        <div className="w-full px-3 sm:px-6 relative z-10">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid lg:grid-cols-2 gap-3 sm:gap-7 max-w-6xl mx-auto">
              {/* CUSTOM SELECTION - Premium Dark Card (NOW FIRST) */}
                  ...
                  <div className="relative p-3 sm:p-7 md:p-8">
                    ...
                    <button
                      onClick={() => {
                        ...
                        setSelectedBonuses([])
                        // Scroll to bonus grid after selection
                        setTimeout(() => {
                          bonusGridRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' })
                        }, 100)
                      }}
              ...
              {/* SARA'S EXPERT SELECTION - Premium Card (NOW SECOND) */}
              <div className="group relative">
                {/* Premium Badge */}
            ...
            {selectedMode && (
              <div ref={bonusGridRef} className="max-w-6xl mx-auto mt-8 sm:mt-12 animate-slide-up-fade">
                {/* Instruction Banner with animated counter */}
                <div className="bg-gradient-to-r from-emerald-500/20 to-green-500/20 border-2 border-emerald-500/50 rounded-xl p-3 sm:p-4 mb-4 sm:mb-6">
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-2">
                    <p className="text-white font-bold text-sm sm:text-base text-center sm:text-left">
                      {selectedMode === 'expert'
                        ? `✨ 4 bonuses pre-selected! Tap ONE more`
                        : `🎯 Select 5 bonuses to unlock checkout`
                      }
                    </p>
                    {/* Animated Value Counter */}
                    <div className="flex items-center gap-2 bg-white/10 rounded-full px-3 py-1.5">
                      <DollarSign className="w-4 h-4 text-emerald-400" />
                      <span className="text-emerald-400 font-black text-lg tabular-nums">${animatedValue}</span>
                      <span className="text-white/60 text-xs">value</span>
                    </div>
                  </div>
                </div>

                {/* Bonus Grid with stagger animation */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-5 mb-6 sm:mb-8 pb-24 sm:pb-28 stagger-children">
                  {BONUS_PRODUCTS.map((bonus) => {
                    ...
                    const wasJustSelected = justSelected === bonus.id
                    ...
                    return (
                      <button
                        ...
                        className={`relative rounded-xl sm:rounded-2xl p-3 sm:p-4 text-left transition-all duration-300 touch-feedback ${
                          isSelected
                            ? `bg-gradient-to-br from-emerald-500/20 to-green-500/10 border-[3px] border-emerald-500 shadow-2xl shadow-emerald-500/30 scale-[1.02] ring-4 ring-emerald-500/20 ${wasJustSelected ? 'animate-selection-pop' : ''}`
                            : isDisabled
                            ? 'bg-gray-800/50 border-2 border-gray-700 opacity-40 cursor-not-allowed grayscale'
                            : 'bg-white/5 border-2 border-white/20 hover:border-luxury-gold/50 hover:bg-white/10 active:scale-[0.98]'
                        }`}
                      >
                        ...
                        {isSelected && (
                          <div className="absolute -top-2 -right-2 sm:-top-3 sm:-right-3 z-20">
                            <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center shadow-lg ${
                              isTopPick ? 'bg-gradient-to-br from-luxury-gold to-luxury-gold-dark' : 'bg-gradient-to-br from-emerald-500 to-green-600'
                            }`}>
                              {isTopPick ? (
                                <Award className="w-4 h-4 sm:w-5 sm:h-5 text-luxury-black" />
                              ) : (
                        ...
                        {!isSelected && !isDisabled && (
                          <div className="absolute -top-2 -right-2 sm:-top-3 sm:-right-3 z-20">
                            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full border-2 border-dashed border-white/40 flex items-center justify-center bg-white/10">
                              <span className="text-white/60 text-xs font-bold">+</span>
                            </div>
                          </div>
                        )}

                        <div className={`aspect-video rounded-lg sm:rounded-xl mb-2 sm:mb-3 overflow-hidden relative border-2 transition-all ${
                          ...
                          {!isSelected && !isDisabled && (
                            <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
                              <span className="text-white/80 text-xs font-bold bg-black/50 px-2 py-1 rounded">TAP TO SELECT</span>
                            </div>
                          )}
                        </div>

                        <div>
                          <div className={`text-xs uppercase mb-1 font-bold ${isSelected ? 'text-emerald-400' : 'text-luxury-gold'}`}>{bonus.category}</div>
                          <h5 className={`font-bold text-xs sm:text-sm mb-1 sm:mb-2 line-clamp-2 ${isSelected ? 'text-white' : 'text-white/80'}`}>{bonus.title}</h5>
                          <p className={`text-xs mb-2 sm:mb-3 line-clamp-2 ${isSelected ? 'text-white/70' : 'text-gray-400'}`}>{bonus.subtitle}</p>

                          <div className="flex items-center gap-2">
                            <span className="text-gray-500 line-through text-sm font-semibold">${bonus.value}</span>
                            <div className={`px-2 py-1 rounded-full text-xs font-black ${
                              isSelected ? 'bg-emerald-500/30 text-emerald-400' : 'bg-luxury-gold/20 text-luxury-gold'
                            }`}>
                        ...
                        {isTopPick && (
                          <div className="mt-2 bg-luxury-gold/20 border border-luxury-gold/40 rounded-lg px-2 py-1.5 text-xs text-luxury-gold font-black text-center">
                            ⭐ Sara's Top Pick
                          </div>
                        )}
                      </button>
                ...
                {/* Checkout CTA - with ref for auto-scroll */}
                <div ref={checkoutButtonRef}>
                  {selectedBonuses.length === 5 && (
                    <div className="bg-gradient-to-br from-emerald-500/20 to-green-500/10 border-2 border-emerald-500 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-2xl shadow-emerald-500/20 animate-bounce-in">
                      <div className="text-center">
                        {/* Success Badge with pulse */}
                        <div className="inline-flex items-center gap-2 bg-emerald-500 text-white px-4 py-2 rounded-full text-sm font-black mb-3 sm:mb-4 animate-success-pulse">
                          <CheckCircle className="w-4 h-4" />
                          Perfect! 5/5 Selected
                        </div>
                        {/* Animated Value Display */}
                        <p className="text-white text-lg sm:text-xl font-black mb-2">
                          <span className="text-emerald-400">${animatedValue}</span> Value Unlocked!
                        </p>
                        <p className="text-white/70 text-sm mb-4">
                          All bonuses included FREE with your purchase
                        </p>
                        <button
                          onClick={openModal}
                          className="w-full bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-400 hover:to-green-500 text-white px-6 py-4 sm:py-5 rounded-xl font-league-spartan font-black text-base sm:text-lg transition-all flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-[0.98] cursor-pointer animate-glow-pulse"
                        >
                          <Zap className="w-5 h-5" />
                          <span>Get Instant Access — $37</span>
                          <ArrowRight className="w-5 h-5" />
                        </button>
                        <p className="text-white/70 text-xs sm:text-sm mt-3 flex items-center justify-center gap-2">
                          <Shield className="w-3 h-3" /> 30-Day Money-Back Guarantee • Secure Checkout
                        </p>
                      </div>
                    </div>
                  )}
                ...
                {/* STICKY Progress Bar - Mobile - Ultra Premium */}
                <div className="fixed bottom-0 left-0 right-0 z-50 p-3 sm:p-4 bg-gradient-to-t from-black via-black/95 to-transparent pointer-events-none safe-bottom">
                  <div className="max-w-lg mx-auto pointer-events-auto">
                    <div className={`backdrop-blur-xl border-2 rounded-2xl p-3 sm:p-4 shadow-2xl transition-all duration-500 ${
                      selectedBonuses.length === 5
                        ? 'bg-gradient-to-r from-emerald-900/90 to-green-900/90 border-emerald-500 animate-glow-pulse'
                        : 'bg-luxury-black/90 border-luxury-gold/30'
                    }`}>
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          <div className="flex gap-1.5">
                            {[...Array(5)].map((_, i) => (
                              <div
                                key={i}
                                className={`w-7 h-7 sm:w-9 sm:h-9 rounded-lg flex items-center justify-center transition-all duration-300 ${
                                  i < selectedBonuses.length
                                    ? 'bg-gradient-to-br from-emerald-500 to-green-600 scale-110 shadow-lg shadow-emerald-500/50'
                                    : 'bg-white/10 border border-white/20'
                                } ${i === selectedBonuses.length - 1 && justSelected ? 'animate-bounce-in' : ''}`}
                              >
                                {i < selectedBonuses.length && <Check className="w-4 h-4 sm:w-5 sm:h-5 text-white" strokeWidth={3} />}
                              </div>
                            ))}
                          </div>
                          <div className="flex flex-col">
                            <span className={`font-black text-lg sm:text-xl leading-none ${selectedBonuses.length === 5 ? 'text-emerald-400' : 'text-white'}`}>
                              {selectedBonuses.length}/5
                            </span>
                            {selectedBonuses.length < 5 && (
                              <span className="text-white/50 text-[10px] sm:text-xs">
                                {5 - selectedBonuses.length} left
                              </span>
                            )}
                          </div>
                        </div>
                        ...
                        {selectedBonuses.length === 5 ? (
                          <button
                            onClick={openModal}
                            className="bg-gradient-to-r from-emerald-500 to-green-600 text-white px-5 sm:px-6 py-3 sm:py-3.5 rounded-xl font-black text-sm sm:text-base flex items-center gap-2 hover:scale-105 active:scale-95 transition-all shadow-lg animate-float touch-feedback"
                          >
                            <Zap className="w-4 h-4" />
                            <span>Checkout $37</span>
                          </button>
                        ) : (
        ...
        <div className="w-full px-3 sm:px-6 lg:px-8 relative z-10">
          ...
          {!selectedMode && (
            <div className="max-w-6xl mx-auto mb-5 sm:mb-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                ...
                {/* DARK THEME - Full Control (NOW FIRST) */}
                <button
                ...
                {/* LIGHT THEME - Sara's Choice (NOW SECOND) */}
                <button
          ...
          {selectedMode && (
            <div className="max-w-6xl mx-auto animate-slide-up-fade">
              {/* STICKY Instruction Banner with animated counter */}
              <div className="sticky top-0 z-50 -mx-3 px-3 sm:-mx-6 sm:px-6 pt-2 pb-3 sm:pb-4 bg-luxury-pearl/95 backdrop-blur-md">
                <div className="bg-gradient-to-r from-emerald-500/20 to-green-500/20 border-2 border-emerald-500/50 rounded-xl p-3 sm:p-4 shadow-lg">
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-2">
                    <p className="text-luxury-black font-bold text-sm sm:text-base text-center sm:text-left">
                      {selectedMode === 'expert'
                        ? `✨ 4 bonuses pre-selected! Tap ONE more`
                        : `🎯 Select 5 bonuses to unlock checkout`
                      }
                    ...
                    {/* Animated Value Counter */}
                    <div className="flex items-center gap-2 bg-emerald-500/20 rounded-full px-3 py-1.5">
                      <DollarSign className="w-4 h-4 text-emerald-600" />
                      <span className="text-emerald-600 font-black text-lg tabular-nums">${animatedValue}</span>
                      <span className="text-emerald-600/60 text-xs">value</span>
                    </div>
                  </div>
                </div>
              </div>

              {/* Bonus Grid - COMPACT for mobile */}
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3 mb-4 pb-28 sm:pb-32 stagger-children">
                {BONUS_PRODUCTS.map((bonus) => {
                  const isSelected = selectedBonuses.includes(bonus.id)
                  const isTopPick = selectedMode === 'expert' && TOP_4_BONUSES.includes(bonus.id)
                  const isDisabled = !isSelected && selectedBonuses.length >= 5
                  ...
                  return (
                    <button
                      ...
                      onClick={() => toggleBonus(bonus.id)}
                      disabled={isDisabled && !isSelected}
                      className={`relative rounded-lg p-2 text-left transition-all duration-200 touch-feedback ${
                        isSelected
                          ? `bg-emerald-500/10 border-2 border-emerald-500 shadow-lg shadow-emerald-500/20 ${wasJustSelected ? 'animate-selection-pop' : ''}`
                          : isDisabled
                          ? 'bg-gray-100 border border-gray-200 opacity-40 cursor-not-allowed grayscale'
                          : 'bg-white border border-gray-200 hover:border-luxury-gold/50 active:scale-[0.98]'
                      }`}
                    >
                      {/* Selection Badge - Top Right - Small */}
                      {isSelected && (
                        <div className="absolute -top-1.5 -right-1.5 z-20">
                          <div className={`w-5 h-5 rounded-full flex items-center justify-center shadow ${
                            isTopPick ? 'bg-luxury-gold' : 'bg-emerald-500'
                          } ${wasJustSelected ? 'animate-bounce-in' : ''}`}>
                      ...
                      {!isSelected && !isDisabled && (
                        <div className="absolute -top-1.5 -right-1.5 z-20">
                          <div className="w-5 h-5 rounded-full border border-dashed border-gray-300 flex items-center justify-center bg-white">
                      ...
                      {/* Small Image */}
                      <div className={`aspect-[4/3] rounded-md mb-1.5 overflow-hidden relative border transition-all ${
                        isSelected ? 'border-emerald-500/50' : 'border-gray-100'
                      }`}>
                        {bonus.image && (
                          <Image src={bonus.image} alt={bonus.title} fill sizes="50vw" className="object-cover" loading="lazy" />
                        )}
                      </div>

                      {/* Compact Info */}
                      <div>
                        <h5 className={`font-bold text-[10px] sm:text-xs leading-tight line-clamp-2 mb-1 ${isSelected ? 'text-emerald-700' : 'text-gray-800'}`}>{bonus.title}</h5>
                        <div className="flex items-center gap-1">
                          <span className="text-gray-400 line-through text-[10px]">${bonus.value}</span>
                          <span className={`text-[9px] font-black ${isSelected ? 'text-emerald-500' : 'text-luxury-gold'}`}>
                            {isSelected ? '✓' : 'FREE'}
                          </span>
                        </div>
                      </div>
                      ...
                      {isTopPick && (
                        <div className="mt-1 bg-luxury-gold/20 rounded px-1 py-0.5 text-[8px] text-luxury-gold font-black text-center">
                          ⭐ TOP PICK
                        </div>
                      )}
                    </button>
              ...
              {/* Ref for potential scroll - hidden since sticky bar handles checkout */}
              <div ref={checkoutButtonRef} className="h-0" />
            </div>
          )}
        </div>
      </section>
      ...
      {/* ORDER SUMMARY MODAL - Fixed scroll issue, redesigned */}
      {isMounted && isModalOpen && createPortal(
        <div
          className="fixed inset-0 z-[9999] bg-black/95 backdrop-blur-sm overflow-y-auto overscroll-contain"
          onClick={closeModal}
        >
          {/* Top padding for red header visibility */}
          <div className="min-h-screen flex items-start justify-center p-3 pt-8 sm:p-4 sm:pt-10">
            <div
              className="relative w-full max-w-lg bg-gradient-to-br from-[#0a0a0a] via-[#111] to-[#0a0a0a] rounded-2xl border border-white/20 shadow-2xl"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Close Button */}
              <button
                onClick={closeModal}
                className="absolute top-3 right-3 z-10 w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white/60 hover:text-white transition-all"
              >
                <X className="w-4 h-4" />
              </button>

              {/* Black Friday Header */}
              <div className="bg-gradient-to-r from-red-600 via-red-500 to-red-600 p-2 text-center rounded-t-2xl">
              ...
              <div className="p-4 space-y-3">
                {/* VIP Badge */}
                <div className="text-center">
                  <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#D4AF37]/10 border border-[#D4AF37]/30">
                    <Sparkles className="w-3 h-3 text-[#D4AF37]" />
                    <span className="text-[10px] font-bold text-[#D4AF37] tracking-widest">VIP PACKAGE</span>
                  </div>
                </div>

                {/* Main Course */}
                <div className="bg-gradient-to-br from-[#D4AF37]/15 to-[#D4AF37]/5 rounded-xl p-3 border border-[#D4AF37]/40">
                  <div className="flex items-center gap-3">
                    <div className="relative w-16 h-16 rounded-lg overflow-hidden flex-shrink-0 border-2 border-[#D4AF37]/40">
                      <Image src="/images/VD-Course-demo.webp" alt="Course" fill className="object-cover" />
                    </div>
                    <div className="flex-1">
                      <h3 className="text-sm font-black text-white">7-Min AgentClone™</h3>
                      <p className="text-[10px] text-white/50">Complete System + Lifetime Access</p>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-white/40 line-through text-sm">$97</span>
                        <span className="text-[#D4AF37] font-black text-xl">$37</span>
                        <span className="bg-green-500/20 text-green-400 text-[9px] px-1.5 py-0.5 rounded font-bold">62% OFF</span>
                      </div>
                    </div>
                  </div>
                </div>

                {/* Selected Bonuses - Better price display */}
                <div className="bg-white/5 rounded-xl p-3 border border-emerald-500/30">
                  <h4 className="text-xs font-black text-white mb-2 flex items-center gap-1.5">
                    ...
                    Your 5 FREE Bonuses
                    <span className="text-emerald-400 ml-auto">${totalValue} value</span>
                  </h4>
                  <div className="space-y-1.5">
                    {selectedBonuses.map((bonusId) => {
                      ...
                      return (
                        <div key={bonus.id} className="flex items-center gap-2 bg-black/40 rounded-lg p-2">
                          {bonus.image && (
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-[11px] font-bold text-white truncate">{bonus.title}</p>
                          </div>
                          <div className="flex items-center gap-2 flex-shrink-0">
                            <span className="text-red-400 line-through text-xs font-bold">${bonus.value}</span>
                            <span className="bg-emerald-500 text-white text-[10px] px-2 py-0.5 rounded font-black">FREE</span>
                          </div>
                        </div>
                      )
                    })}
                ...
                {/* Mystery Box - Premium VIP Style */}
                <div className="bg-gradient-to-r from-violet-600/20 to-purple-600/20 rounded-xl p-3 border border-violet-500/40">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <span className="text-white font-black text-sm">Mystery Box</span>
                        <span className="bg-violet-500/30 text-violet-300 px-1.5 py-0.5 rounded text-[9px] font-black">SECRET</span>
                      </div>
                      <p className="text-white/50 text-[10px]">Exclusive surprise revealed after purchase</p>
                    </div>
                    <div className="text-right">
                      <span className="text-violet-400 font-black text-sm">$500-$1,500</span>
                    </div>
                  </div>
                </div>

                {/* Lifetime Updates - NEW */}
                <div className="bg-gradient-to-r from-cyan-500/15 to-blue-500/15 rounded-xl p-3 border border-cyan-500/40">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <span className="text-white font-black text-sm">Lifetime Updates</span>
                        <span className="bg-cyan-500/30 text-cyan-300 px-1.5 py-0.5 rounded text-[9px] font-black">WEEKLY</span>
                      </div>
                      <p className="text-white/50 text-[10px]">New tools & tips added every week forever</p>
                    </div>
                    <div className="text-right">
                      <span className="text-cyan-400 font-black text-sm">$297</span>
                    </div>
                  </div>
                </div>

                {/* Sara's Guarantee - Irresistible */}
                <div className="bg-gradient-to-br from-[#0d1f17] to-[#0a1a14] rounded-xl p-3 border border-emerald-500/40">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="relative w-12 h-12 rounded-full overflow-hidden border-2 border-emerald-400 flex-shrink-0">
                      <Image src="/images/Sara 61kb.webp" alt="Sara" fill className="object-cover" />
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-white font-black text-sm">Sara's "You Win Either Way" Guarantee</span>
                      </div>
                      <div className="inline-flex items-center gap-1 bg-emerald-500 rounded px-1.5 py-0.5 mt-0.5">
                  ...
                  <div className="space-y-2 text-[11px] text-white/80">
                    <div className="flex items-start gap-2">
                      ...
                      <p><span className="text-white font-bold">30-Day Money-Back:</span> Don't like it? Full refund. No questions.</p>
                    </div>
                    <div className="flex items-start gap-2">
                      <CheckCircle className="w-4 h-4 text-emerald-400 flex-shrink-0 mt-0.5" />
                      <p><span className="text-white font-bold">PLUS $50 Bonus:</span> If these don't help, I'll Venmo you $50.</p>
                    </div>
                    <div className="flex items-start gap-2">
                      <CheckCircle className="w-4 h-4 text-emerald-400 flex-shrink-0 mt-0.5" />
                      <p><span className="text-white font-bold">Keep Everything:</span> Even if you refund, keep all bonuses.</p>
                    </div>
                  </div>
                  <div className="mt-3 bg-emerald-500/20 border border-emerald-500/40 rounded-lg p-2 text-center">
                    <p className="text-emerald-400 text-[11px] font-black">You literally CANNOT lose. Either profit or get paid to try it.</p>
                  </div>
                </div>
                ...
                <div className="pt-2">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-white/40 line-through text-sm">${97 + totalValue + 297 + 500}</span>
                  </div>
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-white font-black text-lg">Today Only:</span>
                    <span className="text-[#D4AF37] font-black text-3xl">$37</span>
                  </div>
                  <button
                    onClick={() => {
                      if (selectedBonuses.length !== 5) {
                    ...
                    className="w-full bg-gradient-to-r from-[#D4AF37] via-[#FFE17B] to-[#D4AF37] hover:shadow-2xl hover:shadow-[#D4AF37]/50 text-black font-black text-lg py-4 rounded-xl transition-all hover:scale-[1.02] active:scale-[0.98]"
                  >
                    Complete My Order - $37
                  </button>
                  <p className="text-[10px] text-center text-white/40 mt-2 flex items-center justify-center gap-1">
                    <Shield className="w-3 h-3" />
                    Secure SSL Encrypted • Instant Access
                  </p>
                </div>
              </div>
            </div>
      ...
      {/* GLOBAL STICKY CHECKOUT - VIP Premium Design - Hide when modal is open */}
      {selectedBonuses.length === 5 && !isModalOpen && typeof document !== 'undefined' && createPortal(
        <div
          style={{
            position: 'fixed',
            bottom: 0,
            ...
            background: 'linear-gradient(to top, rgba(0,0,0,0.98), rgba(0,0,0,0.9), transparent)',
          }}
        >
          <div style={{ maxWidth: '400px', margin: '0 auto' }}>
            ...
            <div
              style={{
                ...
                borderRadius: '20px',
                padding: '16px',
                boxShadow: '0 -8px 40px rgba(212, 175, 55, 0.15), 0 4px 20px rgba(0,0,0,0.8)',
              }}
            >
              {/* Status Badge */}
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '12px' }}>
                ...
                >
                  ...
                  <span style={{ color: '#10b981', fontWeight: 800, fontSize: '13px', letterSpacing: '0.5px' }}>5/5 SELECTED</span>
                  <span style={{ color: '#d4af37', fontWeight: 700, fontSize: '12px', marginLeft: '4px' }}>${animatedValue} VALUE</span>
                </div>
              </div>
      ...
      {/* Progress bar when selecting (less than 5) */}
      {selectedMode && selectedBonuses.length < 5 && (
        <div className="fixed bottom-0 left-0 right-0 z-[10001] p-2 sm:p-3 safe-bottom">
          <div className="max-w-lg mx-auto">
            <div className="bg-white/95 backdrop-blur-xl border border-gray-200 rounded-xl p-2.5 shadow-lg">
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  <div className="w-9 h-9 bg-luxury-gold/20 rounded-lg flex items-center justify-center font-black text-sm text-luxury-gold">
                    {selectedBonuses.length}/5
                  </div>
                  <div>
                    <p className="font-bold text-xs text-gray-800">{5 - selectedBonuses.length} more to go</p>
                    <p className="text-[10px] text-gray-500">${animatedValue} selected</p>
                  </div>
                </div>
                  ...
                  {[...Array(5)].map((_, i) => (
                    <div
                      ...
                      className={`w-2 h-2 rounded-full transition-all ${
                        i < selectedBonuses.length ? 'bg-emerald-500' : 'bg-gray-200'
                      }`}
                    />
                  ))}
                </div>
