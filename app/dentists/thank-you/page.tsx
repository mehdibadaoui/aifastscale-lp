...
function ThankYouContent() {
  ...
  useEffect(() => {
    const visited = sessionStorage.getItem('dentist_thankyou_visited')
    if (visited) {
      setHasVisited(true)
    }
  }, [])
  ...
  const fetchCredentials = useCallback(async (email?: string, userId?: string) => {
    try {
      ...
      const response = await fetch(`/api/dentist-credentials?${params.toString()}`)
  ...
  useEffect(() => {
    const startPolling = async () => {
      const storedEmail = sessionStorage.getItem('dentist_purchase_email')
      const storedUserId = sessionStorage.getItem('dentist_purchase_user_id')
  ...
  if (hasVisited) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#0a0a0a] via-[#0f1f1f] to-[#1a2a2e] flex items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0a0a0a] via-[#0f1f1f] to-[#1a2a2e]">
        ...
        <div className="text-center mb-8">
          ...
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10">
            ...
            <span className="text-white text-sm font-medium">
              You joined <span className="text-teal-400 font-bold">21,000+</span> dentists
            </span>
          </div>
        </div>
