...
import {
  ...
  Download, ChevronLeft, Video, Clock,
  ArrowRight, Star, Trophy, Flame, Target, Crown, Stethoscope
} from 'lucide-react'
import Image from 'next/image'

// DENTIST Product data - using dentist-specific images
const MAIN_COURSE = {
  title: "AI Video Mastery for Dentists",
  items: [
    { id: 'register', name: 'Register Account', type: 'pdf', file: '/products/register-here.pdf', duration: '2 min', image: '/images/dentist/course-demo.webp', description: 'Create your HeyGen account' },
    { id: 'prompt-guide', name: 'Prompt Mastery Guide', type: 'pdf', file: '/products/prompt-guide.pdf', duration: '5 min', image: '/images/dentist/viral-scripts.webp', description: 'Learn to write perfect prompts' },
    { id: 'video-1', name: 'Core Training', type: 'video', file: '/products/training-video-1.mov', duration: '12 min', image: '/images/dentist/social-templates.webp', description: 'Step-by-step video creation' },
    { id: 'video-2', name: 'Advanced Tips', type: 'video', file: '/products/training-video-2.mov', duration: '8 min', image: '/images/dentist/website-template.webp', description: 'Pro techniques & shortcuts' },
    { id: 'template-1', name: 'Script Template A', type: 'doc', file: '/products/prompt-template-1.rtf', duration: '1 min', image: '/images/dentist/front-desk-scripts.webp', description: 'Copy-paste video scripts' },
    { id: 'template-2', name: 'Script Template B', type: 'doc', file: '/products/prompt-template-2.rtf', duration: '1 min', image: '/images/dentist/profit-simulator.webp', description: 'More ready-to-use scripts' },
  ]
}

const BONUSES = [
  { id: 'viral-scripts', name: '100 Viral Dental Scripts', type: 'pdf', file: '/products/dentist/viral-scripts.pdf', image: '/images/dentist/viral-scripts.webp', cat: 'tool', description: '100 ready-to-use video scripts' },
  { id: 'social-templates', name: '365 Days Social Content', type: 'pdf', file: '/products/dentist/social-templates.pdf', image: '/images/dentist/social-templates.webp', cat: 'template', description: 'Full year of content' },
  { id: 'website-template', name: 'High-Converting Website', type: 'pdf', file: '/products/dentist/website-template.pdf', image: '/images/dentist/website-template.webp', cat: 'template', description: 'Proven dental website design' },
  { id: 'profit-simulator', name: 'Profit Simulator', type: 'pdf', file: '/products/dentist/profit-simulator.pdf', image: '/images/dentist/profit-simulator.webp', cat: 'tool', description: 'Find hidden profit in practice' },
  { id: 'front-desk-scripts', name: 'Front Desk Scripts', type: 'pdf', file: '/products/dentist/front-desk-scripts.pdf', image: '/images/dentist/front-desk-scripts.webp', cat: 'tool', description: 'Convert more phone calls' },
  { id: 'patient-forms', name: 'Patient Forms Bundle', type: 'pdf', file: '/products/dentist/patient-forms.pdf', image: '/images/dentist/patient-forms.webp', cat: 'template', description: 'Professional patient forms' },
  { id: 'referral-machine', name: 'Referral Machine', type: 'pdf', file: '/products/dentist/referral-machine.pdf', image: '/images/dentist/referral-machine.webp', cat: 'tool', description: 'Turn patients into referrals' },
  { id: 'ai-generator', name: 'AI Content Generator', type: 'pdf', file: '/products/dentist/ai-generator.pdf', image: '/images/dentist/ai-generator.webp', cat: 'tool', description: '500+ dental prompts' },
  { id: 'review-explosion', name: '5-Star Review System', type: 'pdf', file: '/products/dentist/review-explosion.pdf', image: '/images/dentist/review-explosion.webp', cat: 'training', description: 'Get 15+ reviews monthly' },
  { id: 'before-after-kit', name: 'Before/After Kit', type: 'pdf', file: '/products/dentist/before-after-kit.pdf', image: '/images/dentist/before-after-kit.webp', cat: 'training', description: 'Viral transformation content' },
]
      ...
      {[...Array(60)].map((_, i) => (
        <div
          ...
          style={{
            ...
            backgroundColor: ['#14b8a6', '#06b6d4', '#10b981', '#22d3ee', '#ffffff', '#0d9488'][Math.floor(Math.random() * 6)],
...
function AchievementBadge({ icon: Icon, label, unlocked }: { icon: any; label: string; unlocked: boolean }) {
  return (
    <div className={`flex flex-col items-center gap-1 p-2 rounded-xl transition-all duration-300 ${
      unlocked ? 'bg-teal-500/20 scale-100' : 'bg-white/5 scale-95 opacity-50'
    }`}>
      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
        unlocked ? 'bg-teal-500 text-white' : 'bg-white/10 text-white/30'
      }`}>
        <Icon className="w-4 h-4" />
      </div>
      <span className={`text-[8px] font-bold ${unlocked ? 'text-teal-400' : 'text-white/30'}`}>{label}</span>
    </div>
  )
}

export default function DentistMembersPage() {
  ...
  const correctPassword = 'im the best dentist in the world'
  ...
  useEffect(() => {
    const saved = localStorage.getItem('dentist_downloaded')
    if (saved) setDownloaded(JSON.parse(saved))
    if (localStorage.getItem('dentistMemberUnlocked') === 'true') setIsUnlocked(true)

    // Streak tracking
    const lastVisitDate = localStorage.getItem('dentistLastVisit')
    const savedStreak = parseInt(localStorage.getItem('dentistStreak') || '0')
    ...
    if (lastVisitDate) {
      ...
      if (diffDays === 1) {
        ...
        localStorage.setItem('dentistStreak', String(savedStreak + 1))
      } else if (diffDays === 0) {
        setStreak(savedStreak)
      } else {
        setStreak(1)
        localStorage.setItem('dentistStreak', '1')
      }
    } else {
      setStreak(1)
      localStorage.setItem('dentistStreak', '1')
    ...
    localStorage.setItem('dentistLastVisit', today)
    setLastVisit(today)
  ...
  useEffect(() => {
    if (downloaded.length) localStorage.setItem('dentist_downloaded', JSON.stringify(downloaded))
  ...
  const handleUnlock = (e: React.FormEvent) => {
    ...
    if (passwordInput.toLowerCase().trim() === correctPassword) {
      setIsUnlocked(true)
      localStorage.setItem('dentistMemberUnlocked', 'true')
      playSound('unlock')
      vibrate([50, 30, 100])
    } else {
  ...
  const copyLink = () => {
    navigator.clipboard.writeText('https://aifastscale.com/dentists/members')
    setCopied(true)
    vibrate(30)
  ...
  // Password Gate - TEAL THEME
  if (!isUnlocked) {
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center px-4">
        <div className="max-w-sm w-full">
          <div className="w-20 h-20 bg-gradient-to-br from-teal-500 to-cyan-500 rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-2xl shadow-teal-500/30 animate-pulse">
            <Lock className="w-10 h-10 text-white" />
          </div>
          <h1 className="text-3xl font-black text-white text-center mb-2">Welcome, Doctor!</h1>
          ...
          <form onSubmit={handleUnlock} className="space-y-4">
            <div className="relative">
              <input
                ...
                className={`w-full bg-white/5 border-2 ${passwordError ? 'border-red-500 animate-shake' : 'border-white/10'} rounded-2xl px-5 py-4 text-white text-base placeholder-white/30 outline-none focus:border-teal-500 text-center transition-all duration-300`}
              />
              {passwordError && (
            ...
            <button
              type="submit"
              className="w-full bg-gradient-to-r from-teal-500 to-cyan-500 text-white py-4 rounded-2xl font-black text-base shadow-xl shadow-teal-500/30 active:scale-[0.97] transition-all duration-200 hover:shadow-2xl hover:shadow-teal-500/40"
            >
              UNLOCK MY COURSE
            </button>
          </form>
      ...
      <>
        ...
        <div className="fixed inset-0 z-[300] bg-black/95 flex items-center justify-center px-4">
          <div className="text-center animate-scaleIn">
            <div className="w-24 h-24 bg-gradient-to-br from-teal-500 to-cyan-500 rounded-full flex items-center justify-center mx-auto mb-6 shadow-2xl shadow-teal-500/50 animate-bounce">
            ...
            <div className="flex justify-center gap-2 mb-6">
              {[...Array(5)].map((_, i) => (
                <Star key={i} className="w-8 h-8 text-teal-400 fill-teal-400 animate-pulse" style={{ animationDelay: `${i * 0.1}s` }} />
              ))}
            </div>
            <p className="text-teal-400 text-sm font-bold">Now go create amazing videos!</p>
          </div>
        </div>
      </>
    )
    ...
    return (
      <div className="fixed inset-0 z-[200] bg-black flex flex-col animate-slideUp">
        <div className="flex items-center justify-between px-4 py-4 bg-[#0a0a0a] border-b border-white/10">
          ...
          <a href={viewer.url} download className="bg-teal-500 text-white px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 active:scale-95 transition-transform">
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] pb-20" style={{ paddingBottom: 'calc(80px + env(safe-area-inset-bottom))' }}>
      ...
      <header className="sticky top-0 z-40 bg-[#0a0a0a]/98 backdrop-blur-md border-b border-white/5 px-4 py-3">
        ...
        {/* Animated Progress Bar - TEAL */}
        <div className="mt-3 h-1.5 bg-white/10 rounded-full overflow-hidden">
          <div
            className="h-full bg-gradient-to-r from-teal-500 via-emerald-400 to-teal-500 rounded-full transition-all duration-700 ease-out relative overflow-hidden"
      ...
      <main className="px-4 py-4">
        ...
        {activeTab === 'home' && (
          <div className="space-y-5 animate-fadeIn">
            {/* Welcome Message - TEAL */}
            <div className="bg-gradient-to-br from-teal-500/20 via-teal-500/10 to-transparent border border-teal-500/30 rounded-2xl p-5">
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 bg-gradient-to-br from-teal-500 to-cyan-500 rounded-xl flex items-center justify-center flex-shrink-0 shadow-lg shadow-teal-500/20">
                  {progressPercent === 100 ? <Crown className="w-6 h-6 text-white" /> : <Sparkles className="w-6 h-6 text-white" />}
                </div>
                <div>
                  <h2 className="text-white font-bold text-base mb-1">
                    {progressPercent === 0 ? `${getGreeting()}, Doctor! Let's begin` :
            ...
            {nextItem && progressPercent > 0 && progressPercent < 100 && (
              <button
                ...
                className="w-full bg-gradient-to-r from-teal-500/20 to-teal-500/5 border border-teal-500/30 rounded-2xl p-4 flex items-center gap-4 active:scale-[0.98] transition-all duration-200"
              >
                ...
                <div className="flex-1 text-left">
                  <p className="text-[10px] text-teal-400 font-bold uppercase tracking-wider">Continue</p>
                    ...
                    return (
                      ...
                      >
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${
                          done ? 'bg-emerald-500 text-white' : 'bg-teal-500 text-white'
                        }`}>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => switchTab('course')} className="text-teal-400 text-xs font-bold flex items-center gap-1 active:opacity-70">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => switchTab('bonuses')} className="text-teal-400 text-xs font-bold flex items-center gap-1 active:opacity-70">
                  See All <ChevronRight className="w-3 h-3" />
                </button>
              </div>
              <div className="grid grid-cols-2 gap-3">
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-4 p-4 rounded-2xl border-2 transition-all duration-200 active:scale-[0.98] ${
                      ...
                      isNext ? 'bg-teal-500/10 border-teal-500/40 shadow-lg shadow-teal-500/10' :
                  ...
                  >
                    ...
                    <div className="flex-1 text-left">
                      {isNext && <p className="text-[10px] text-teal-400 font-bold uppercase tracking-wider mb-0.5">Up Next</p>}
                    ...
                    <ChevronRight className={`w-6 h-6 ${done ? 'text-emerald-400' : isNext ? 'text-teal-400' : 'text-white/20'}`} />
                  </button>
                )
              })}
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="p-3 bg-[#0a0a0a]">
                      ...
                      <p className={`text-[10px] font-bold mt-1.5 ${done ? 'text-emerald-400' : 'text-teal-400'}`}>
          ...
          <div className="space-y-4 animate-fadeIn">
            ...
            <div className="space-y-3">
              {[
                ...
                { q: 'What if I have questions about dental marketing?', a: 'Our support team can help with any dental-specific questions. Email us anytime!' },
              ].map((f, i) => (
            ...
            <div className="bg-gradient-to-br from-teal-500/20 to-teal-500/5 border border-teal-500/30 rounded-2xl p-5">
              ...
              <a
                ...
                className="block w-full bg-gradient-to-r from-teal-500 to-cyan-500 text-white py-3.5 rounded-xl text-sm font-bold text-center active:scale-[0.97] transition-transform shadow-lg shadow-teal-500/20"
              >
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center gap-1 py-2 px-5 rounded-2xl transition-all duration-200 ${
                  isActive ? 'text-teal-400 bg-teal-500/10' : 'text-white/30'
                }`}
              >
