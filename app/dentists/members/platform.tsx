...
const COURSE_MODULES = [
  {
    ...
    description: 'Your roadmap to creating AI videos that attract patients',
    duration: '8 min',
    wistiaId: 'myh13ayp9d',
    thumbnail: '/images/dentist/course-1.webp',
  ...
  },
  {
    ...
    duration: '12 min',
    wistiaId: 'lpkrdgr19e',
    thumbnail: '/images/dentist/course-2.webp',
  ...
  },
  {
    ...
    duration: '15 min',
    wistiaId: 'n4lx79ecbo',
    thumbnail: '/images/dentist/course-3.webp',
    lessons: ['Writing magnetic scripts', 'Recording your video', 'Export & optimization'],
    resources: [
      { name: 'Dentist AI Talking Video Prompt (Universal)', url: '/products/dentist/Dentist_AI_Talking_Video_Prompt_Universal.pdf', type: 'file' as const },
    ],
  },
  {
    id: 'module-4',
    ...
    duration: '10 min',
    wistiaId: 'j2reo83n4z',
    thumbnail: '/images/dentist/course-4.webp',
  ...
  },
  {
    ...
    description: 'Turn views into booked appointments with proven strategies',
    duration: '14 min',
    wistiaId: null,
    thumbnail: '/images/dentist/course-5.webp',
  ...
  },
  {
    ...
    duration: '11 min',
    wistiaId: null,
    thumbnail: '/images/dentist/course-6.webp',
...
const MOTIVATIONAL_QUOTES = [
  ...
  { threshold: 100, quote: "You did it! Now go attract those patients!", author: "Your AI Coach" },
]
  ...
  return (
    <button
      ...
      className={`w-full text-left p-4 rounded-xl transition-all duration-300 group ${
        isActive
          ? isDark
            ? 'bg-gradient-to-r from-teal-500/20 to-cyan-500/20 border-2 border-teal-500 shadow-lg shadow-teal-500/20'
            : 'bg-gradient-to-r from-teal-50 to-cyan-50 border-2 border-teal-500 shadow-lg shadow-teal-500/10'
          : isCompleted
            ? isDark
              ? 'bg-slate-800/50 border border-teal-500/50 hover:border-teal-500'
            ...
            : isLocked || module.comingSoon
              ...
              : isDark
                ? 'bg-slate-800/50 border border-slate-700 hover:border-teal-500/50 hover:bg-slate-800'
                : 'bg-white border border-slate-200 hover:border-teal-300 hover:shadow-md shadow-sm'
      }`}
    >
  ...
  return (
    <div className={`p-4 rounded-xl transition-all duration-300 ${
      isDark
        ...
        : 'bg-white border border-slate-200 hover:border-teal-300 shadow-sm hover:shadow-md'
    }`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between">
            ...
            {bonus.file ? (
              <button
                onClick={onClick}
                className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium bg-gradient-to-r from-teal-500 to-cyan-500 text-white hover:opacity-90 transition-opacity"
              >
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className={`relative max-w-sm w-full rounded-2xl p-8 text-center ${
        ...
        <button
          onClick={onClose}
          className="w-full py-3 rounded-xl font-bold text-white bg-gradient-to-r from-teal-500 to-cyan-500 hover:opacity-90 transition-opacity"
        >
          Keep Going!
        </button>
      </div>
      ...
      <div className={`min-h-screen flex items-center justify-center p-4 ${
        ...
        <div className={`relative w-full max-w-md rounded-3xl p-8 ${
          ...
          <div className="text-center mb-8">
            <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-teal-500 to-cyan-500 flex items-center justify-center shadow-lg shadow-teal-500/30">
              <GraduationCap className="w-8 h-8 text-white" />
            </div>
            <h1 className={`text-2xl font-black mb-1 ${isDark ? 'text-white' : 'text-slate-900'}`}>
              {COURSE_CONFIG.title}
            </h1>
            <p className="text-teal-500 font-bold">{COURSE_CONFIG.subtitle}</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3 rounded-xl border-2 transition-all duration-200 ${
                  isDark
                    ? 'bg-slate-800/90 border-slate-600 text-white placeholder-slate-500 focus:border-teal-400'
            ...
            <button
              type="submit"
              className="w-full py-3 rounded-xl font-bold text-white bg-gradient-to-r from-teal-500 to-cyan-500 hover:opacity-90 transition-all duration-200 shadow-lg shadow-teal-500/30"
            >
              Access My Course
            </button>
          </form>
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="hidden sm:block">
                ...
                <p className="text-xs text-teal-500">{COURSE_CONFIG.subtitle}</p>
              </div>
            </div>
        ...
        <div className={`rounded-2xl p-6 mb-8 ${
          ...
          <div className="flex flex-col md:flex-row items-center gap-6">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <div className={`p-4 rounded-xl ${isDark ? 'bg-slate-700/50' : 'bg-teal-50'}`}>
                <p className={`text-sm italic ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                  "{currentQuote.quote}"
                </p>
                <p className="text-xs text-teal-500 mt-1">— {currentQuote.author}</p>
              </div>
            </div>
            {/* Quick stats */}
            <div className="flex gap-4 md:flex-col">
              <div className={`px-4 py-3 rounded-xl text-center ${isDark ? 'bg-slate-700/50' : 'bg-slate-100'}`}>
                <div className="text-2xl font-bold text-teal-500">{ACHIEVEMENTS.filter(a => a.condition([...completedModules, ...completedBonuses], totalLessons)).length}</div>
                <div className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Badges</div>
              </div>
              <div className={`px-4 py-3 rounded-xl text-center ${isDark ? 'bg-slate-700/50' : 'bg-slate-100'}`}>
                <div className="text-2xl font-bold text-teal-500">{BONUSES.length}</div>
                ...
                <div className={`rounded-2xl overflow-hidden ${
                  ...
                  <div className="p-6">
                        ...
                        <button
                          ...
                          className={`flex-shrink-0 flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all ${
                            completedModules.includes(activeModuleData.id)
                              ? 'bg-teal-500/20 text-teal-500 cursor-default'
                      ...
                      <ul className="space-y-2">
                        {activeModuleData.lessons.map((lesson, i) => (
                          <li key={i} className="flex items-center gap-2">
                            <CircleCheck className="w-4 h-4 text-teal-500 flex-shrink-0" />
                    ...
                    {activeModuleData.resources && activeModuleData.resources.length > 0 && (
                      <div className={`p-4 rounded-xl mt-4 ${isDark ? 'bg-teal-500/10 border border-teal-500/30' : 'bg-teal-50 border border-teal-200'}`}>
                        <h4 className="font-medium mb-3 flex items-center gap-2">
