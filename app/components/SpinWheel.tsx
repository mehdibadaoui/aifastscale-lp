      ...
      >
        ...
        <div className="p-8 relative">
          {!showResult ? (
            ...
            <div className="relative">
              <div ref={productExplanationRef} className="relative z-10 px-2 md:px-6 py-3 md:py-6 space-y-3 md:space-y-5 max-h-[85vh] overflow-y-auto">
                ...
                <div className="bg-gradient-to-br from-white/10 via-white/5 to-transparent backdrop-blur-lg border-2 border-gold-premium/30 rounded-2xl p-5 md:p-6 shadow-2xl">
                  ...
                  {/* Bundle Visualization - Styled Grid */}
                  <div className="relative rounded-xl overflow-hidden shadow-2xl mb-4 md:mb-6 bg-gradient-to-br from-purple-900/40 via-navy-rich to-pink-900/30 border-2 border-gold-premium/30 p-2 md:p-4">
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-3">
                      {selectedBonuses.slice(0, 5).map((bonusId, idx) => {
                        ...
                        return (
                          <div key={idx} className="relative aspect-square rounded-lg overflow-hidden border border-gold-premium/30 bg-white/5">
                            {bonus?.image ? (
                      ...
                      {/* Mystery Box */}
                      <div className="relative aspect-square rounded-lg overflow-hidden border-2 border-gold-premium">
                        <Image src="/images/products/Mystery Box.jpeg" alt="Mystery VIP Box" fill className="object-cover" />
                      </div>
                    </div>
                    <p className="text-center text-gold-premium/80 text-xs font-semibold">5 Premium Products + Mystery VIP Box</p>
                  </div>

                  {/* Selected Products Breakdown */}
                  <div className="space-y-2 md:space-y-3 mb-3 md:mb-4">
                    <p className="text-gold-premium font-bold text-xs md:text-sm uppercase text-center mb-2 md:mb-3 flex items-center justify-center gap-2">
                    ...
                    {selectedBonuses.map((bonusId, index) => {
                      ...
                      return (
                        <div key={bonusId} className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-sm border border-white/10 rounded-lg p-2.5 md:p-4">
                          <div className="flex items-center gap-2.5 md:gap-3 mb-1.5 md:mb-2">
                            {/* Product Image - smaller on mobile */}
                            {bonus.image && (
                              <div className="flex-shrink-0 w-12 h-12 md:w-16 md:h-16 rounded-lg overflow-hidden shadow-lg">
                                <Image
                                  src={bonus.image}
                                  alt={bonus.title}
                                  width={64}
                            ...
                            {!bonus.image && (
                              <div className="flex items-center justify-center flex-shrink-0 bg-gradient-to-br from-white/10 to-white/5 rounded-lg shadow-md w-12 h-12 md:w-16 md:h-16">
                                <ProductIcon product={bonus} size="w-5 h-5 md:w-7 md:h-7" />
                              </div>
                            )}
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between gap-1">
                                <h4 className="font-bold text-white text-xs md:text-sm leading-tight line-clamp-2">{bonus.title}</h4>
                                <span className="text-emerald-400 font-bold text-xs md:text-sm whitespace-nowrap">${bonus.value}</span>
                              </div>
                            </div>
                          </div>
                          <p className="text-white/60 text-[10px] md:text-xs leading-relaxed line-clamp-3 md:line-clamp-none">{bonus.description}</p>
                        </div>
                      )
                    })}
                ...
                <div className="bg-gradient-to-br from-emerald-500/20 to-emerald-600/10 border-2 border-emerald-400/50 rounded-2xl p-4 md:p-5 backdrop-blur-sm">
                  {/* Header with Badge */}
                  <div className="text-center mb-4">
                    <div className="inline-flex items-center gap-2 bg-emerald-500/20 border border-emerald-400/40 rounded-full px-4 py-1.5 mb-2">
                      <Sparkles className="w-4 h-4 text-emerald-400" />
                      <span className="text-emerald-400 font-bold text-xs uppercase tracking-wider">Triple Guarantee Protection</span>
                    </div>
                    ...
                    <p className="text-emerald-300/80 text-sm mt-1">We remove ALL risk from your decision</p>
                  </div>

                  {/* Compact 3-Column Guarantees */}
                  <div className="grid grid-cols-3 gap-2 mb-4">
                    <div className="bg-white/5 rounded-lg p-3 text-center border border-white/10">
                      <RefreshCw className="w-7 h-7 text-emerald-400 mx-auto mb-1" />
                      <p className="font-bold text-white text-[11px] md:text-xs">30-Day Money Back</p>
                      <p className="text-white/60 text-[9px] md:text-[10px] mt-0.5">Don't like it? Full refund. No questions. No hassle.</p>
                    </div>
                    <div className="bg-white/5 rounded-lg p-3 text-center border border-white/10">
                      <Gift className="w-7 h-7 text-emerald-400 mx-auto mb-1" />
                      <p className="font-bold text-white text-[11px] md:text-xs">Keep The Bonuses</p>
                      <p className="text-white/60 text-[9px] md:text-[10px] mt-0.5">Even if you refund, you keep ALL bonuses ($191+ value)</p>
                    </div>
                    <div className="bg-white/5 rounded-lg p-3 text-center border border-white/10">
                      <Zap className="w-7 h-7 text-emerald-400 mx-auto mb-1" />
                      <p className="font-bold text-white text-[11px] md:text-xs">Instant Access</p>
                      <p className="text-white/60 text-[9px] md:text-[10px] mt-0.5">Start in 60 seconds. Lifetime updates included free.</p>
                    </div>
                  </div>

                  {/* Sara's Promise - Modern Card with Image */}
                  <div className="bg-gradient-to-r from-gold-premium/10 to-gold-premium/5 border border-gold-premium/30 rounded-xl p-3 md:p-4">
                    <div className="flex items-center gap-3">
                      {/* Sara's Image */}
                      <div className="relative w-14 h-14 md:w-16 md:h-16 rounded-full overflow-hidden border-2 border-gold-premium flex-shrink-0">
                        <Image src="/images/Sara 61kb.webp" alt="Sara" fill className="object-cover object-top" />
                      </div>
                      <div className="flex-1">
                        <p className="text-white text-xs md:text-sm leading-relaxed">
                          <span className="font-black text-gold-premium">Here's my promise:</span> If this doesn't help you create stunning AI videos in 7 minutes or less,
                  ...
                  <div className="flex items-center justify-center flex-wrap gap-x-3 gap-y-1 mt-3 pt-3 border-t border-white/10 text-white/50 text-[10px]">
                    <span className="flex items-center gap-1"><Check className="w-3 h-3 text-emerald-400" /> Secure checkout</span>
                    <span className="flex items-center gap-1"><Check className="w-3 h-3 text-emerald-400" /> Instant delivery</span>
                    <span className="flex items-center gap-1"><Check className="w-3 h-3 text-emerald-400" /> No recurring fees</span>
                    <span className="flex items-center gap-1"><Check className="w-3 h-3 text-emerald-400" /> Support included</span>
                  </div>
                </div>

                {/* ⭐ COMPACT TESTIMONIAL */}
                <div className="bg-white/5 backdrop-blur-sm border border-gold-premium/20 rounded-xl p-3">
                  <div className="flex items-center gap-3">
                    <div className="flex-shrink-0 w-10 h-10 rounded-full bg-gradient-to-br from-gold-premium to-yellow-600 flex items-center justify-center text-lg">
                      👨‍💼
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1 mb-0.5">
                        {[1,2,3,4,5].map(i => (
                          <Star key={i} className="w-3 h-3 fill-gold-premium text-gold-premium" />
                        ))}
                      </div>
                      ...
                      <p className="text-gold-premium text-[10px] font-bold">- Marcus J., Real Estate Agent, Miami FL</p>
                    </div>
                  </div>
                </div>
                ...
                <button
                  onClick={() => {
                    ...
                    trackInitiateCheckout('AI Video Mastery Course', 37.0)
