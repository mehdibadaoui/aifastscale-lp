...
/**
 * Psychologist Members Authentication API
 *
  ...
  try {
    ...
    if (trimmedEmail) {
      ...
      if (result.success && result.user) {
        // Verify user is for psychologist product
        if (result.user.product !== 'psychologist') {
          console.log(`⚠️ User ${trimmedEmail} tried to access psychologist area but has ${result.user.product} product`)
          return NextResponse.json({
            success: false,
            error: 'This login is for a different product. Please use the correct members area.'
          }, { status: 401 })
        }

        console.log(`✅ Psychologist user logged in: ${trimmedEmail}`)
        return NextResponse.json({
          success: true,
          isVip: false,
          user: {
    ...
    console.error('Psychologist auth error:', error)
    return NextResponse.json(
...
export async function GET() {
  return NextResponse.json({
    status: 'Psychologist auth endpoint active',
