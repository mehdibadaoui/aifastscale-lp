...
/**
 * Meta Conversions API for Plastic Surgeons
 *
 * Fires server-side events to Meta for accurate tracking.
 * Used for Purchase events from webhook (can't be blocked by browsers).
 */

const PIXEL_ID = '1526841625273321'
const ACCESS_TOKEN = process.env.PLASTIC_SURGEON_META_CAPI_TOKEN || ''
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!ACCESS_TOKEN) {
      console.log('⚠️ PLASTIC_SURGEON_META_CAPI_TOKEN not configured')
      return NextResponse.json(
    ...
    const eventData = {
      ...
      event_source_url: sourceUrl || 'https://aifastscale.com/plastic-surgeons',
    ...
    if (response.ok) {
      console.log(`✅ Plastic Surgeon CAPI ${eventName}: $${value || 0} - ${contentName || 'N/A'}`)
      return NextResponse.json({
        success: true,
    ...
    console.error('❌ Plastic Surgeon CAPI Exception:', error)
    return NextResponse.json(
...
export async function GET() {
  return NextResponse.json({
    status: 'Plastic Surgeon Meta CAPI endpoint active',
    pixelId: PIXEL_ID,
