...
/**
 * Meta Conversions API for Dentists
 *
 * Fires server-side events to Meta for accurate tracking.
 * Used for ViewContent, InitiateCheckout, and Purchase events.
 * Server-side tracking can't be blocked by browsers/iOS privacy.
 *
 * Pixel ID: 1176362697938270 (Dentist-specific)
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!ACCESS_TOKEN) {
      console.log('⚠️ DENTIST_META_CAPI_TOKEN not configured')
      return NextResponse.json(
    ...
    if (response.ok) {
      console.log(`✅ Dentist CAPI ${eventName}: $${value || 0} - ${contentName || 'N/A'}`)
      return NextResponse.json({
        success: true,
      ...
      console.error('❌ Dentist CAPI Error:', result)
      return NextResponse.json(
    ...
    console.error('❌ Dentist CAPI Exception:', error)
    return NextResponse.json(
...
export async function GET() {
  return NextResponse.json({
    status: 'Dentist Meta CAPI endpoint active',
    pixelId: PIXEL_ID,
