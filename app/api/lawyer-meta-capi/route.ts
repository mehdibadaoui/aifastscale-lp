...
/**
 * Meta Conversions API for Lawyers
 *
...
// Lawyer Pixel ID
const PIXEL_ID = '2660533017679840'
const ACCESS_TOKEN = process.env.LAWYER_META_CAPI_TOKEN || ''
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!ACCESS_TOKEN) {
      console.log('⚠️ LAWYER_META_CAPI_TOKEN not configured')
      return NextResponse.json(
    ...
    const eventData = {
      ...
      event_source_url: sourceUrl || 'https://aifastscale.com/lawyers',
    ...
    if (response.ok) {
      console.log(`✅ Lawyer CAPI ${eventName}: $${value || 0} - ${contentName || 'N/A'}`)
      return NextResponse.json({
        success: true,
    ...
    console.error('❌ Lawyer CAPI Exception:', error)
    return NextResponse.json(
...
export async function GET() {
  return NextResponse.json({
    status: 'Lawyer Meta CAPI endpoint active',
    pixelId: PIXEL_ID,
