...
/**
 * Lawyer Members Authentication API
 *
  ...
  try {
    ...
    if (trimmedEmail) {
      ...
      if (result.success && result.user) {
        // Verify user is for lawyer product
        if (result.user.product !== 'lawyer') {
          console.log(`⚠️ User ${trimmedEmail} tried to access lawyer area but has ${result.user.product} product`)
          return NextResponse.json({
            success: false,
        ...
        console.log(`✅ Lawyer user logged in: ${trimmedEmail}`)
        return NextResponse.json({
          success: true,
          isVip: false,
          user: {
    ...
    console.error('Lawyer auth error:', error)
    return NextResponse.json(
...
export async function GET() {
  return NextResponse.json({
    status: 'Lawyer auth endpoint active',
