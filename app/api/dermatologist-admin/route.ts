...
// GET - Fetch all dermatologist members data
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const dermatologistEmails = await redis.smembers('users:dermatologist')

    const members: User[] = []
    for (const email of dermatologistEmails) {
      const user = await redis.get<User>(`user:${email}`)
      if (user && user.product === 'dermatologist') {
        if (!user.revenue) {
    ...
    console.error('Dermatologist Admin API error:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'create') {
      ...
      const newPassword = `clone-${randomPart}`
      ...
      await redis.sadd('users:dermatologist', normalizedEmail)
    ...
    if (action === 'resend') {
      ...
      await resend.emails.send({
        ...
        to: email,
        subject: '🔑 Your Login Details (Resent) - CloneYourself for Dermatologists',
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto; padding: 20px;">
            <h2 style="color: #e11d48;">Your Login Credentials</h2>
            <p>Here are your login details for CloneYourself for Dermatologists:</p>
            <div style="background: #fff1f2; padding: 20px; border-radius: 12px; margin: 20px 0; border: 1px solid #e11d48;">
              <p style="margin: 8px 0;"><strong>Email:</strong> ${user.email}</p>
              <p style="margin: 8px 0;"><strong>Password:</strong> <code style="background: #ffe4e6; padding: 4px 8px; border-radius: 4px; font-size: 16px;">${user.password}</code></p>
            </div>
            <p><a href="https://aifastscale.com/members" style="background: #e11d48; color: white; padding: 14px 28px; text-decoration: none; border-radius: 8px; display: inline-block; font-weight: bold;">Login to Members Area</a></p>
        ...
        try {
          ...
          if (user) {
            await resend.emails.send({
              ...
              replyTo: 'support@aifastscale.com',
              to: em,
              subject: '🔑 Your Login Details - CloneYourself for Dermatologists',
              html: `
                <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto; padding: 20px;">
                  <h2 style="color: #e11d48;">Your Login Credentials</h2>
                  <div style="background: #fff1f2; padding: 20px; border-radius: 12px; margin: 20px 0; border: 1px solid #e11d48;">
                    <p style="margin: 8px 0;"><strong>Email:</strong> ${user.email}</p>
                    <p style="margin: 8px 0;"><strong>Password:</strong> <code style="background: #ffe4e6; padding: 4px 8px; border-radius: 4px;">${user.password}</code></p>
                  </div>
                  <p><a href="https://aifastscale.com/members" style="background: #e11d48; color: white; padding: 14px 28px; text-decoration: none; border-radius: 8px; display: inline-block; font-weight: bold;">Login to Members Area</a></p>
                </div>
              `
    ...
    if (action === 'bulkDelete') {
      ...
      for (const em of emails) {
        ...
        if (user) {
          ...
          await redis.srem('users:dermatologist', normalizedEmail)
          deleted++
        }
      }
    ...
    console.error('Dermatologist Admin POST error:', error)
