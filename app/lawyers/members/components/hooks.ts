...
const STORAGE_PREFIX = 'lawyer_v4_'
...
export function usePlatformState() {
  ...
  useEffect(() => {
    ...
    if (DEV_AUTO_LOGIN) {
      ...
      const savedAuthData = localStorage.getItem('lawyerMemberAuth')
      ...
      if (savedAuthData) {
        try {
          ...
          if (authData.expiry && Date.now() < authData.expiry) {
            ...
            // Expired - clear auth
            localStorage.removeItem('lawyerMemberAuth')
          }
        } catch {
      ...
      if (!isValidAuth) {
        const sessionAuth = sessionStorage.getItem('lawyerMemberAuth')
  ...
  const handleLogin = useCallback(async (email: string, password: string, rememberMe: boolean = true): Promise<boolean> => {
    ...
    try {
      const response = await fetch('/api/lawyer-auth', {
        method: 'POST',
      ...
      if (data.success) {
        ...
        if (rememberMe) {
          ...
          localStorage.setItem('lawyerMemberAuth', JSON.stringify({
            authenticated: true,
            expiry: thirtyDaysFromNow
          }))
        } else {
          // Session only - use sessionStorage (cleared when browser closes)
          sessionStorage.setItem('lawyerMemberAuth', 'true')
          localStorage.removeItem('lawyerMemberAuth')
        }
        ...
        if (data.isVip && data.vipGuest) {
          ...
          localStorage.setItem('lawyerMemberEmail', data.user.email)
        } else {
          setVipGuest(null)
        }
  ...
  const handleLogout = useCallback(() => {
    ...
    setVipGuest(null)
    localStorage.removeItem('lawyerMemberAuth')
    sessionStorage.removeItem('lawyerMemberAuth')
