...
import { getMemberStats, computeDerivedConfig } from '../../config'
...
import { PremiumCourse } from '../course/PremiumCourse'
...
// ============================================
// RESPONSIVE PLATFORM WRAPPER
// ============================================

interface ResponsivePlatformProps {
  state: PlatformStateType
  children: React.ReactNode
}

export const ResponsivePlatformWrapper = memo(function ResponsivePlatformWrapper({
  state,
  children
}: ResponsivePlatformProps) {
  const isMobile = useIsMobile()

  // On mobile, render the MobilePlatform with bottom navigation
  if (isMobile) {
    return (
      <MobilePlatform
        state={{
          ...
          markModuleComplete: (id: string) => {
            // Find the module index and mark it complete
            const moduleIndex = state.config.modules.findIndex((m: any) => m.id === id)
            if (moduleIndex !== -1) {
          ...
          achievements: state.config.achievements.map((a: any) => ({
            ...a,
            unlocked: state.unlockedAchievements[a.id] || false
          }))
        }}
      />
    )
  ...
  // On desktop, render children (the existing desktop sections)
  return <>{children}</>
})
...
export const DashboardSection = memo(function DashboardSection({ state }: DashboardProps) {
  ...
  // Use the new Premium Dashboard component
  return (
    <PremiumDashboard
      state={{
        studentName: state.studentName,
...
// ============================================
// LEGACY DASHBOARD SECTION (PRESERVED FOR REFERENCE)
// ============================================

const LegacyDashboardSection = memo(function LegacyDashboardSection({ state }: DashboardProps) {
  const { config } = state
  const derived = useMemo(() => computeDerivedConfig(config), [config])

  const [showWelcomeVideo, setShowWelcomeVideo] = useState(false)
  const [memberStats, setMemberStats] = useState(() => getMemberStats(config.memberStats))

  useEffect(() => {
    const interval = setInterval(() => {
      setMemberStats(getMemberStats(config.memberStats))
    ...
    <div className="space-y-4 sm:space-y-8 animate-fade-in">
      ...
      <div className="sm:hidden grid grid-cols-2 gap-3">
        {[
          ...
          { icon: Trophy, label: 'Badges', value: `${state.unlockedCount}/${config.achievements.length}`, color: 'from-emerald-500 to-amber-500', iconColor: '#10b981' },
        ].map((stat, i) => (
      ...
      <div className="hidden sm:grid grid-cols-3 lg:grid-cols-5 gap-4">
        ...
        {[
          ...
          { icon: Trophy, label: 'Badges', value: `${state.unlockedCount}/${config.achievements.length}`, color: 'from-emerald-500 to-amber-500' },
        ].map((stat, i) => (
      ...
      {/* MOBILE: Welcome Video Prompt - LP Glass Style */}
      {state.completedCount === 0 && !showWelcomeVideo && (
        <div className="sm:hidden">
          <div className="rounded-2xl glass-premium p-4 border border-amber-500/20">
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-amber-500 to-yellow-500 flex items-center justify-center flex-shrink-0 ring-2 ring-amber-500/50 shadow-lg">
                <Sparkles className="w-8 h-8 text-white" />
              </div>
              <div className="flex-1">
                <p className="text-[10px] font-bold text-amber-400 uppercase tracking-wider">Start Here</p>
                <p className="text-sm font-bold text-white">Welcome to Your Training</p>
                <button
      ...
      {/* DESKTOP: Welcome Video Prompt - LP Glass Style */}
      {state.completedCount === 0 && !showWelcomeVideo && (
        <div className="hidden sm:block glass-premium rounded-3xl p-6 border border-amber-500/20">
          <div className="flex items-center gap-6">
            <div className="w-24 h-24 rounded-2xl bg-gradient-to-br from-amber-500 to-yellow-500 flex items-center justify-center flex-shrink-0 shadow-xl ring-4 ring-amber-500/30">
              <Sparkles className="w-12 h-12 text-white" />
            </div>
            <div className="flex-1">
              <div className="text-xs font-bold text-amber-400 uppercase tracking-wider mb-1">Start Here</div>
              <h3 className="text-xl font-black text-white mb-2">Welcome to Your Training</h3>
              <button
                onClick={() => setShowWelcomeVideo(true)}
        ...
        <div className="glass-premium rounded-3xl shadow-2xl border border-amber-500/20 overflow-hidden">
          <div className="p-4 border-b border-white/10 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-bold text-white">Welcome Video</div>
                <div className="text-sm text-slate-400">Getting Started Guide</div>
              </div>
            </div>
          ...
          <WistiaPlayer wistiaId={config.welcomeVideo?.wistiaId || 'fnjt6devuy'} showSpeedControls={false} />
        </div>
      )}
      ...
      {/* Value Reinforcement Banner - Anti-Refund Feature */}
      <div className="hidden sm:block">
        <TotalValueBadge config={config} />
      </div>

      {/* MOBILE: Value Badge */}
      <div className="sm:hidden flex justify-center">
        <TotalValueBadge config={config} compact />
      </div>

      {/* Reviews & Updates Grid - Anti-Refund Features */}
      <div className="grid lg:grid-cols-2 gap-6">
        {/* Member Success Stories */}
        <ReviewsSection state={state} />

        {/* What's New & Coming This Week */}
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-slate-400 mt-1">New courses coming soon. Join the waitlist!</p>
            </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {config.comingSoonCourses.map((course) => {
...
export const CourseSection = memo(function CourseSection({ state }: CourseProps) {
  // Use the new Premium Course component with all the enhanced features
  return (
    <PremiumCourse
      state={{
        ...
        setCurrentModuleIndex: state.setCurrentModuleIndex,
        handleVideoProgress: state.handleVideoProgress,
...
// ============================================
// LEGACY COURSE SECTION (PRESERVED FOR REFERENCE)
// ============================================

const LegacyCourseSection = memo(function LegacyCourseSection({ state }: CourseProps) {
  const { config } = state
  ...
  const canGoNext = state.currentModuleIndex < state.totalModules - 1 && !config.modules[state.currentModuleIndex + 1]?.comingSoon
    ...
    <div {...swipeHandlers} className="space-y-4 sm:space-y-6 animate-fade-in touch-pan-y">
        ...
        <div className="space-y-4">
          <div className="glass-premium rounded-2xl p-5 border border-amber-500/20">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs text-slate-400 font-medium">{state.completedCount}/{config.modules.filter(m => !m.comingSoon).length}</span>
            </div>
            <div className="space-y-3">
              {config.modules.filter(m => !m.comingSoon).map((module, index) => {
                const moduleCompleted = state.completedModules.includes(module.id)
                const moduleIndex = config.modules.findIndex(m => m.id === module.id)
      ...
      <div className="sm:hidden space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-slate-400">{state.completedCount}/{config.modules.filter(m => !m.comingSoon).length} completed</span>
        </div>

        <div className="space-y-4">
          {/* Show all modules EXCEPT the current one, including coming soon */}
          {config.modules.filter(m => {
            const moduleIndex = config.modules.findIndex(mod => mod.id === m.id)
...
export const BonusesSection = memo(function BonusesSection({ state }: BonusesProps) {
  // Use the new Premium Bonuses component
  return (
    <PremiumBonuses
      state={{
        downloadedBonuses: state.downloadedBonuses,
...
// ============================================
// LEGACY BONUSES SECTION (PRESERVED FOR REFERENCE)
// ============================================

const LegacyBonusesSection = memo(function LegacyBonusesSection({ state }: BonusesProps) {
  const { config } = state
  const derived = useMemo(() => computeDerivedConfig(config), [config])
  ...
  const filteredBonuses = config.bonuses.filter(bonus => {
    ...
    <div className="space-y-4 sm:space-y-6 animate-fade-in">
      ...
      <div className="sm:hidden">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="px-3 py-1.5 rounded-full bg-gradient-to-r from-amber-500 to-yellow-500 text-white text-xs font-bold shadow-glow-gold">
            ${derived.totalBonusValue} Value
          </div>
        </div>
        <p className="text-sm text-slate-400">{config.bonuses.length} premium resources included</p>
      </div>
      ...
      <div className="hidden sm:flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 mt-1">Resources worth <span className="font-bold text-amber-400">${derived.totalBonusValue}</span></p>
        </div>
        <div className="text-right">
          <p className="text-slate-500 text-sm">{state.downloadedBonuses.length}/{config.bonuses.length} downloaded</p>
        </div>
      </div>

      {/* MOBILE: Category Pills - LP Glass Style */}
      <div className="sm:hidden flex gap-2 overflow-x-auto pb-2 -mx-4 px-4 scrollbar-hide">
        {derived.bonusCategories.map((cat) => (
          <button
            key={cat}
    ...
    <div className="space-y-4 sm:space-y-6 animate-fade-in">
      ...
      <div className="sm:hidden">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="px-3 py-1.5 rounded-full bg-gradient-to-r from-amber-500 to-orange-500 text-white text-xs font-bold shadow-lg">
            {state.unlockedCount}/{config.achievements.length}
          </div>
        </div>
      ...
      <div className="hidden sm:flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 mt-1">{state.unlockedCount} of {config.achievements.length} unlocked • {state.totalPoints} points</p>
        </div>
      </div>
