...
export function FloatingActionButton({ actions }: FABProps) {
  ...
  return (
    <div className="fixed bottom-20 right-4 z-40 md:hidden pb-safe">
      ...
      <button
        onClick={toggleOpen}
        className={`w-14 h-14 rounded-full bg-gradient-to-r from-amber-500 to-yellow-500 text-white shadow-glow-gold flex items-center justify-center transition-all duration-300 ${isOpen ? 'rotate-45 scale-110' : ''}`}
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Notes" icon={<StickyNote className="w-5 h-5 text-amber-500" />}>
      ...
      <div className="p-4 border-t border-amber-500/20 flex gap-3 justify-end">
        ...
        <button onClick={handleSave} className="btn-premium px-4 py-2 rounded-xl font-bold text-white shadow-glow-gold transition-all btn-press focus-ring">
          Save Notes
        </button>
      </div>
    </Modal>
...
export function KeyboardShortcutsModal({ isOpen, onClose, shortcuts }: { isOpen: boolean; onClose: () => void; shortcuts?: { key: string; action: string }[] }) {
  ...
  const keyboardShortcuts = shortcuts || defaultShortcuts
  return (
...
export function CertificateModal({ isOpen, onClose, studentName, completionDate, courseTitle = 'CloneYourself', courseSubtitle = 'for Professionals', totalRuntime = 23, shareUrl = 'https://aifastscale.com' }: CertificateModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Your Certificate" size="lg" showPrint>
      <div className="p-8">
        <div className="bg-gradient-to-br from-amber-900/40 to-yellow-900/40 rounded-2xl p-8 border-4 border-amber-500/30 text-center relative overflow-hidden glass-premium">
          {/* Decorative elements */}
          ...
          <div className="absolute top-1/2 right-0 w-4 h-20 bg-gradient-to-l from-yellow-500/20 to-transparent -translate-y-1/2" />
          ...
          <div className="relative z-10">
            ...
            <h1 className="text-4xl font-black gradient-text-animated mb-2">{courseTitle}</h1>
            <p className="text-xl text-amber-400 font-bold mb-8">{courseSubtitle}</p>
    ...
    if (this.state.hasError) {
      return (
        <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-premium noise-overlay section-premium relative overflow-hidden">
          ...
          <div className="absolute bottom-20 left-10 w-96 h-96 bg-gradient-to-tr from-yellow-500/10 to-amber-500/5 rounded-full blur-3xl floating" />
          ...
          <div className="text-center max-w-md animate-fade-in glass-premium p-8 rounded-2xl border border-amber-500/20">
            ...
            <button
              ...
              className="btn-premium px-6 py-3 rounded-xl font-bold text-white shadow-glow-gold transition-all btn-press"
            >
              Refresh Page
            </button>
          </div>
  ...
  if (isMobile) {
    return (
      <div className="fixed inset-0 overflow-hidden pointer-events-none z-0" aria-hidden="true">
        ...
        <div className="absolute bottom-0 left-0 w-48 h-48 bg-gradient-to-tr from-yellow-500/10 to-amber-500/10 rounded-full blur-3xl" />
      </div>
    )
  }
