  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-navy-deep via-navy-rich to-black text-white overflow-x-hidden">
        ...
        <div className="space-y-4 mb-6">
          ...
          >
            ...
            <span className="relative flex items-center justify-center gap-3">
              ...
              <span>Yes! I Want Results Like Mr. Lucas - $63</span>
              <Check className="w-7 h-7" />
            </span>
          </button>
      ...
      <WhopCheckoutModal
        ...
        onComplete={async () => {
          // Track OTO purchase after successful payment
          try {
            await fetch('/api/track-purchase', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                selectedBonuses: [], // OTO has no bonus selection
                spinGift: null,
            ...
            console.log('✅ OTO purchase tracked!')
          } catch (error) {
            console.log('⚠️ OTO purchase tracking failed (non-critical):', error)
          }

          // Redirect to thank you page
          window.location.href = '/thank-you-confirmed?purchased=oto'
        }}
      />
    </div>
  )
